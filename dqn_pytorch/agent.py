...
class Agent():
    ...
    def run(self, is_training=True, render=False):
        ...
        if is_training:
            ...
            log_message = f"{start_time.strftime(DATE_FORMAT)}: Training starting in console mode..."
            print(log_message)
            with open(self.LOG_FILE, 'a') as file:  # Changed from 'w' to 'a' to append
                file.write(log_message + '\n')

        # For training: always use console mode
        # For testing: use visual mode if render=True
        if is_training:
            render_mode = None
            print(f"🖥️  Training in console mode...")
        else:
            render_mode = 'human' if render else None
            if render:
                print(f"🎮 Testing with visual mode...")
            else:
                print(f"🖥️  Testing in console mode...")
        ...
        if is_training:
            ...
            # Load existing model if it exists (continue training from previous session)
            if os.path.exists(self.MODEL_FILE):
                print(f"📁 Loading existing model from {self.MODEL_FILE}")
                policy_dqn.load_state_dict(torch.load(self.MODEL_FILE))
                target_dqn.load_state_dict(torch.load(self.MODEL_FILE))
                print(f"✅ Model loaded successfully - continuing from previous training session")
                
                # Try to read the best reward from the log file
                best_reward = -9999999
                        ...
                        with open(self.LOG_FILE, 'r') as file:
                            ...
                            for line in reversed(lines):
                                if "New best reward" in line:
                                    # Extract reward value from log line
                                    reward_str = line.split("New best reward ")[1].split(" ")[0]
                                    best_reward = float(reward_str)
                                    print(f"📊 Resuming training from best reward: {best_reward}")
                                    break
                    except Exception as e:
                        print(f"⚠️  Could not read previous best reward from log: {e}")
                        best_reward = -9999999
            else:
        ...
        for episode in itertools.count():
            ...
            if is_training and episode % 10 == 0:
                print(f"Episode {episode}: Reward = {episode_reward:.1f}, Epsilon = {epsilon:.4f}, Memory size = {len(memory)}")

            # Save model when new best reward is obtained.
            if is_training:
                # Calculate percentage improvement
                if best_reward <= 0:
                    # For negative rewards, use absolute improvement
                    improvement_percentage = abs((episode_reward - best_reward) / best_reward * 100) if best_reward != 0 else 100
                else:
                    # For positive rewards, use standard percentage
                    improvement_percentage = (episode_reward - best_reward) / best_reward * 100
                
                # Save model every time reward improves
                if episode_reward > best_reward:
                    log_message = f"{datetime.now().strftime(DATE_FORMAT)}: New best reward {episode_reward:0.1f} ({improvement_percentage:+.1f}%) at episode {episode}, saving model..."
                    print(log_message)
